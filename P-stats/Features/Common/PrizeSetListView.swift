...
struct PrizeSetListView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showAdd) {
            PrizeSetEditSheet(name: $newName, balls: $newBalls, title: "新規追加") {
                let b = Int(newBalls) ?? 1500
                let name = newName.trimmingCharacters(in: .whitespaces)
                let label = name.isEmpty ? "\(b)玉" : name
                let ps = PrizeSet(name: label, balls: b)
