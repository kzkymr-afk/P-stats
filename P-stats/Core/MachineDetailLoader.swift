...
/// index.json と machines/[id].json を取得し、MachineDetail 用にデコードする。
...
enum MachineDetailLoader {
    ...
    static func fetchIndex(baseURL: String? = nil) async -> [MachineDetailIndexEntry]? {
        ...
        if let url = indexURL(baseURL: base) {
            do {
                ...
                let list = try decoder.decode([MachineDetailIndexEntry].self, from: data)
        ...
        if let data = Bundle.main.url(forResource: "index", withExtension: "json", subdirectory: "master_out")
            ...
            if let list = try? decoder.decode([MachineDetailIndexEntry].self, from: data) {
    ...
    /// 指定した machine_id の machines/[id].json を取得し、MachineDetail にデコードする。失敗時は nil。ネット失敗時はバンドル内があればそれを返す。
    static func fetchMachineDetail(machineId: String, baseURL: String? = nil) async -> MachineDetail? {
        let base = baseURL?.trimmingCharacters(in: .whitespaces).isEmpty == false
            ? baseURL!
            : PresetServiceConfig.defaultMachineDetailBaseURL
        let safe = sanitizeMachineId(machineId)
        if let url = machineDetailURL(baseURL: base, machineId: machineId) {
            do {
                let (data, _) = try await session.data(from: url)
                let detail = try decoder.decode(MachineDetail.self, from: data)
                return detail
            } catch {
    ...
    /// 機種ID または機種名から MachineDetail を取得する。masterID が空でも機種名で index を検索して取得を試みる。
    /// - Parameters:
    ...
    /// - Returns: 取得した MachineDetail。取得できない場合は nil。
    static func fetchMachineDetail(machineId: String?, machineName: String?, baseURL: String? = nil) async -> MachineDetail? {
        let base = baseURL?.trimmingCharacters(in: .whitespaces).isEmpty == false
            ? baseURL!
            : PresetServiceConfig.defaultMachineDetailBaseURL
