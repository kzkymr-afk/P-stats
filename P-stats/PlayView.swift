...
struct PlayView: View {
    ...
    @State private var showGesturePlayUI = false
    ...
    @State private var returnToGesturePlayAfterExit = false
    ...
    /// 波紋の色：公式ボーダー基準の5段階。±0.5=白、+0.5〜+1.5=水色、+1.5超=青、-0.5〜-1.5=黄オレンジ、-1.5未満=赤
    private var expectationBorderColor: Color {
        guard borderForGauge > 0, log.effectiveUnitsForBorder > 0 else { return .white }
        let border = borderForGauge
        let rate = log.realRate
        let diff = rate - border
        if diff > 1.5 { return Color(red: 0.2, green: 0.45, blue: 1.0) }       // 青
        if diff > 0.5 { return Color(red: 0, green: 0.83, blue: 1.0) }       // 水色
        if diff >= -0.5 { return Color.white }                                 // 白
        if diff >= -1.5 { return Color(red: 1.0, green: 0.65, blue: 0.2) }    // 黄オレンジ
        return Color(red: 1.0, green: 0.25, blue: 0.25)                       // 赤
    }
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            let h2 = H * 0.05   // ヘッダー
            let h20info = H * 0.20  // 情報群（ゲージ高さ＝右側パネル高さに合わせる）
            let h10 = H * 0.10  // 大当たり履歴
            let h22center = H * 0.24 // カウント・現金・持ち玉（情報群つまり分を吸収）
            let barHeight = min(H * 0.22, 108)  // RUSH・通常・終了を大きく（ヘッダー縮小分を回す）
        ...
        .sheet(isPresented: $showWinInputSheet) {
            WinInputSheetView(
                winType: tempWinType,
                ...
                onConfirm: {
                    ...
                    log.setRotationsTo(rot)
                    log.addWin(type: tempWinType, atRotation: rot)
                    showWinInputSheet = false
                    if returnToGesturePlayAfterExit {
                        if tempWinType == .normal {
                            showGesturePlayUI = true
                            returnToGesturePlayAfterExit = false
                            return
                        }
                    }
                    if tempWinType == .rush {
                    ...
                    if tempWinType == .normal, log.selectedMachine.timeShortRotations > 0 {
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) { showChanceMode = true }
                    }
                },
                onCancel: {
                    showWinInputSheet = false
        ...
        .fullScreenCover(isPresented: $showGesturePlayUI) {
            GesturePlayView(
                log: log,
                rightHandMode: rightHandMode,
                onExit: { showGesturePlayUI = false },
                onOpenRush: {
                    returnToGesturePlayAfterExit = true
                    showGesturePlayUI = false
                    tempWinType = .rush
                    tempWinRotation = "\(log.totalRotations)"
        ...
        .onAppear {
            ...
            if playViewStartWithPowerSaving, !didAutoOpenPowerSavingThisSession {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { showGesturePlayUI = true }
            }
        }
        .onDisappear {
    ...
    // 左＝ゲージ（仕様: 行全体の高さで表示、上半円・2R×Rでクリップ）、右＝総回転数〜持ち玉のパネル
    @ViewBuilder
    private func infoRow(height: CGFloat) -> some View {
        ...
        HStack(alignment: .top, spacing: 10) {
            BorderMeterView(
                ...
                normalRotations: log.normalRotations,
                formulaBorderLabel: log.dynamicBorder > 0 ? String(format: "%.1f", log.dynamicBorder) : (log.selectedMachine.border.isEmpty ? "—" : log.selectedMachine.border),
                accent: focusAccent
            )
                ...
                infoStatPanel {
                    HStack {
                        Text("期待値")
                            ...
                            .foregroundColor(focusAccent.opacity(0.7))
                        Spacer()
                        Text(log.dynamicBorder > 0 && log.effectiveUnitsForBorder > 0 ? String(format: "%.2f", log.expectationRatio) : "—")
                            .font(.system(size: 15, weight: .bold, design: .monospaced))
                            .foregroundColor(focusAccent)
                    }
                    .padding(.horizontal, 10)
    ...
    /// トップページ風：半透明＋角丸＋角度で変わる枠線。色味は tint でうっすら付ける
    ...
    /// 情報群・大当たり履歴・スワイプバー・ボタン群の間隔（すべて一定）
    private let sectionGap: CGFloat = 12

    // 30%: 現金 | 持ち玉 | カウント（トップ風グラスボタン・均等に細い隙間）。右手モード時は左右入れ替え
    @ViewBuilder
    private func centerActionRow(geo: GeometryProxy, height: CGFloat) -> some View {
        let investmentColumn = VStack(spacing: buttonGap) {
            zoneButton(
                tint: .red,
                content: {
                    VStack(spacing: 4) {
    ...
    /// 下部ボタン群（RUSH・通常・終了）の間隔（外枠パネルなし・詰め気味）
    private let bottomBarSpacing: CGFloat = 4
    ...
    private func floatingBottomBar(geo: GeometryProxy, barHeight: CGFloat) -> some View {
        ...
        let rightColumnWidth: CGFloat = contentWidth / 2
        let bottomPadding: CGFloat = 8

        VStack(spacing: 0) {
            HStack(alignment: .top, spacing: bottomBarSpacing) {
                if rightHandMode {
                    normalAndEndColumn(width: rightColumnWidth, height: barHeight, curveRadius: buttonCornerRadius)
        ...
        .confirmationDialog("実戦を終了", isPresented: $showEndConfirm, titleVisibility: .visible) {
            Button("保存して終了") { saveCurrentSession(); dismiss() }
        ...
        Button(action: { handleWinSelection(type: .rush); OrganicHaptics.playRushHeartbeat() }) {
            ZStack {
                RoundedRectangle(cornerRadius: buttonCornerRadius)
                    .fill(playPanelBackground)
                RoundedRectangle(cornerRadius: buttonCornerRadius)
                    .fill(Color.red.opacity(playPanelTintOverlayOpacity))
                Text("RUSH")
            ...
            .overlay(RoundedRectangle(cornerRadius: buttonCornerRadius).stroke(glassStroke(tint: .red), lineWidth: playPanelStrokeLineWidth))
        }
        .buttonStyle(.plain)
        .clipShape(RoundedRectangle(cornerRadius: buttonCornerRadius))
    }

    private func normalAndEndColumn(width: CGFloat, height: CGFloat, curveRadius: CGFloat) -> some View {
            ...
            Button(action: { handleWinSelection(type: .normal); haptic(.medium) }) {
                ZStack {
                    RoundedRectangle(cornerRadius: buttonCornerRadius)
                        .fill(playPanelBackground)
                    RoundedRectangle(cornerRadius: buttonCornerRadius)
                        .fill(Color.blue.opacity(playPanelTintOverlayOpacity))
                    Text("通常\n大当たり")
                        .font(.system(size: 15, weight: .bold, design: .monospaced))
                ...
                .frame(maxWidth: .infinity, maxHeight: .infinity)
                .overlay(RoundedRectangle(cornerRadius: buttonCornerRadius).stroke(glassStroke(tint: .blue), lineWidth: playPanelStrokeLineWidth))
            }
            .buttonStyle(.plain)
    ...
    private func saveCurrentSession() {
        ...
        // 保存後はログをリセット（次の実戦のため）
        // log.reset() などの関数をGameLogに作っておくと便利です
    }
}
...
// MARK: - ボーダーメーター（横バー偏差表示。マーカーは実質回転率、下向き三角・目盛-5/±0/+5のみ数値）
struct BorderMeterView: View {
    ...
    var body: some View {
        GeometryReader { g in
            ...
            VStack(spacing: 0) {
                // 横バーの上：公式ボーダー（ラベル＋交換率加味した数値）。白・実質回転率と同じフォントサイズ
                VStack(spacing: 2) {
                    Text("公式ボーダー")
...
/// 大当たり入力：当選時ゲーム数・投資額（現金・持ち玉）・持ち玉数
struct WinInputSheetView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    TextField("当選時のゲーム数", text: $rotation)
                        .keyboardType(.numberPad)
                    TextField("投資額（現金）円", text: $cashYen)
                        .keyboardType(.numberPad)
                    TextField("投資額（持ち玉）玉", text: $holdingsBalls)
                        .keyboardType(.numberPad)
                    TextField("持ち玉数", text: $holdingsCount)
                        .keyboardType(.numberPad)
                } header: {
                    Text(winType == .rush ? "RUSH大当たり" : "通常大当たり")
                } footer: {
                    Text("データランプ・台の表示に合わせて入力してください")
                }
            }
            .navigationTitle("大当たり入力")
...
struct SyncInputView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Button("確定", action: onConfirm).buttonStyle(.borderedProminent)
            Spacer()
        }.padding().onAppear { DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { focus.wrappedValue = fieldType } }.presentationDetents([.height(280)])
    }
}
