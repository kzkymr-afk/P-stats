...
- **情報行（infoRow）**: 画面の **20%**（`h20info`）。左＝ゲージ（半円）、右＝総回転数・期待値・RUSH・通常・総現金投資・持ち玉の各パネル。
...
- **下部バー（RUSH・通常・遊技終了）**: **min(画面22%, 108pt)**（`barHeight`）。外枠パネルなし。ボタン間隔は **4pt**（`bottomBarSpacing`）。
...
- **外枠**: RUSH・通常・遊技終了を包む**外枠パネルはなし**（以前の ultraThinMaterial の枠は削除済み）。
